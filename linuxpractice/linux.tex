...
are affected by product structure (legacy product) in Linux kernel.
...
The number of committers compared to the number of authors represents
how the contribution team is organized and suggests
a congruency of work load and communication in the team.

Using code change history and developer interviews we describe 
the evolution of contribution practice in the diverse circumstances 
and find that 
...
b) Product structure has a dramatic effect on the organization of contribution
...
have team fluctuating at size 20 (one committer works for ten authors), and 
close coupled modules like kernel would have a size of 10.
c) The product structure is shaped by business requirements and active
contribution inflow.
...
beginning, e.g., substantial expanding of kernel code and increasing of commercial participation.
In particular, different modules of Linux kernel present different nature
...
and devoted substantial effort in their cause.
...
Therefore, it is of interest to understand if the contribution practice
of different modules of Linux kernel differs from each other, 
...
\section{Product Structure and Metrics}
...
on latest code changes. In fact, given the nature of the products such strategies
 make considerable sense. Project A was very difficult to install and run and 
did not have well defined states, making execution logs less valuable for debugging. 
On the other hand, Product C could be intuitively thought of as a state machine 
with well defined states and transitions, suggesting that execution logs represented 
a nearly optimal way to understand the nature of a problem. Therefore, we hypothesize 
that the activity structure is in fact a part of the product structure that is 
either enforced by the particular product domain and its architecture, or encoded 
in the historic information repositories (code, execution traces, and tracking 
systems) of how the product was constructed and maintained in the past. 
...
%developers decrease their productivity but increase their ownership (on files).
...
\begin{figure}
\centering
\includegraphics[height=2.1in, width=3.5in]{avgchgerfile.eps}
\caption{Code ownership}
\label{fig:fileownership}
\end{figure}

\begin{figure}
\centering
\includegraphics[height=2.1in, width=3.5in]{avgchgerfile2.eps}
\caption{Boxplot of code ownership}
\label{fig:ownershipbox}
\end{figure}
...
month and the resulting time series of this measure of code
ownership are shown in Figure~\ref{fig:fileownership}. 
We can see a pronounced increase of this measure from 2005 to 2008, 
then a decrease until today. The boxplot in Figure~\ref{fig:ownershipbox} 
...
\begin{figure}
\centering
\includegraphics[height=2.1in, width=3.5in]{avgfiledvpr2.eps} %10*6
\caption{Boxplot of developer productivity}
\label{fig:productivity}
\end{figure}

At the same time, we found the average number of different files touched 
by a developer per month decreases over time  as shown in Figure~\ref{fig:productivity}, 
this may be due to the fact that the code is becoming more and more complicated,
and therefore tasks are 
becoming more and more difficult. It may require more effort to work on multiple files,
...
Moreover, the average number of modules a developer touches decrease 
over time, together with the average number of changes committed by a developer (author).

In summary, we have the following observation:

\begin{observation}\label{o:2}
\emph{The kernel developers have reduced their scope of activities (number of different 
files they touched per month), but their
code ownership is intensified (number of people who touched a file per month).} 
\end{observation}

This may suggest that they put more effort on single files
